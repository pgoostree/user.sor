...
func TestShouldDeleteGroup(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("an error %s was no expected when opening a stub db connection", err)
	}
	...
	mock.ExpectQuery("users.delete_group").
		WithArgs(groupName).
		WillReturnRows(rows)

	if _, err = grouprepo.DeleteGroup(db, groupName); err != nil {
		t.Errorf("error was not expected while deleting group: %s", err)
	}
